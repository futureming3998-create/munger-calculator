...
# 2. 逻辑计算与自动诊断
        if growth_rate > 0:
            # 计算回归年数，如果市盈率已经达标则设为 0
            years = math.log(current_pe / target_pe) / math.log(1 + growth_rate) if current_pe > target_pe else 0
            
            # 根据你的标准进行自动分类诊断
            if current_pe <= target_pe:
                st.success(f"🌟 **诊断：极具吸引力（黄金坑）**")
                st.write(f"当前 P/E ({current_pe:.2f}) 已低于目标值 ({target_pe})，属于绝对低估区，复利机器在为你白干！")
            elif years < 3:
                st.success(f"✅ **诊断：极具吸引力**")
                st.write(f"回归年数仅为 **{years:.2f}** 年。即使目前有溢价，强大的利润增长也能在极短时间内填平它。")
            elif 3 <= years <= 7:
                st.info(f"⚖️ **诊断：合理区间**")
                st.write(f"回归年数 **{years:.2f}** 年。这是一个好公司值得持有的标准价格，适合长期伴随成长。")
            else:
                st.warning(f"⚠️ **诊断：目前明显过热**")
                st.write(f"回归年数长达 **{years:.2f}** 年。即使公司再优秀，现在的价格也透支了太多增长，建议保持耐心。")
